    ...
    return (
      <div className="flex-1 flex items-center justify-center text-red-400">
        <div className="max-w-xl text-center px-6">
          ...
          <p className="text-white/70">Có lỗi xảy ra khi lấy danh sách bài hát. Vui lòng thử lại sau.</p>
        </div>
      </div>
    );
  }
  ...
  return (
    <main className="flex-1 overflow-y-auto px-6 py-8 text-white">
      <section className="mb-8">
        <div className="flex flex-col gap-3 md:flex-row md:items-end md:justify-between">
          <div>
            ...
            <p className="mt-3 max-w-2xl text-white/70">Khám phá danh sách bài hát mới nhất, nghe thử và phát ngay chỉ với một cú click.</p>
          </div>
        </div>
      </section>
      ...
      <section className="grid gap-6 sm:grid-cols-2 xl:grid-cols-3">
        {songs.map((song) => (
          <article key={song.id} className="group overflow-hidden rounded-3xl border border-white/10 bg-white/5 shadow-[0_20px_60px_rgba(0,0,0,0.18)] transition hover:-translate-y-1 hover:border-violet-400/20">
            <div className="relative overflow-hidden bg-slate-900">
              <img src={song.imgUrl} alt={song.title} className="h-56 w-full object-cover transition duration-300 group-hover:scale-105" />
              <div className="absolute inset-x-0 bottom-0 bg-gradient-to-t from-black/80 to-transparent p-4">
                <p className="text-sm text-white/80">{song.artist}</p>
                <h2 className="text-xl font-semibold text-white line-clamp-2">{song.title}</h2>
              </div>
            </div>
